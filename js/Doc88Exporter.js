...
$(function () {
  ...
  Viewer._Bw(0);
  // 加载全部
  for (let i = 0; i < totalPage; ++i) Viewer._eW(i + 1, 0);
  document.documentElement.style.scrollBehavior = "smooth";
  const splitHeight = document.body.scrollHeight / totalPage;
  let index = 1;
  // 等待加载完成
  const timer = setInterval(() => {
    if (index < totalPage) window.scrollTo(0, index++ * splitHeight);
    const contents = Viewer._p7;
    if (contents.length < totalPage) return;
